...
class Settings:
    ...
    # The episode's illustration: one continuous SVG stroke, revealed while
    # the audio plays. Generated by a call of its own, deliberately off the
    # generation path - see illustration.py. Nothing here is consulted while
    # an episode is being written or spoken.
    #
    # Its own model setting because it is a different job from writing a
    # script and the right model for it is an open question. Defaults to the
    # episode model so a deployment that says nothing gets one it already
    # trusts, rather than one nobody chose.
    illustration_model: str = field(
        default_factory=lambda: os.environ.get("ILLUSTRATION_MODEL", "")
        or os.environ.get("MODEL", "claude-sonnet-5")
    )
    # A path is a few hundred coordinates. This is generous for a detailed
    # drawing and still far below the script budget, which is the point: an
    # illustration must never be able to cost more than the episode.
    illustration_max_tokens: int = _env_int("ILLUSTRATION_MAX_TOKENS", 4000)
    # Draw at all. Off switches the endpoint to an honest empty answer rather
    # than removing it, so a client can always ask and always gets a reason.
    illustration_enabled: bool = field(
        default_factory=lambda: os.environ.get("ILLUSTRATION_ENABLED", "1")
